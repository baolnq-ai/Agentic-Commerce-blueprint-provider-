...
/**
 * Fetch product catalog from merchant backend.
 */
export async function getCatalogProducts(limit = 24): Promise<Product[]> {
  ...
  const data = await handleResponse<ProductAPIResponse[]>(response);
  return data.map((product) => ({
    id: product.id,
    sku: product.sku,
    name: product.name,
    description: `Live catalog product ${product.sku}`,
    basePrice: product.base_price,
    stockCount: product.stock_count,
    minMargin: product.min_margin,
