...
const INITIAL_MESSAGES: ChatMessageType[] = [
  {
    id: "msg_intro_1",
    role: "agent",
    content: "I loaded products from the live merchant catalog. Pick one to start checkout.",
...
export function AgentPanel({ protocol }: AgentPanelProps) {
  const [messages] = useState<ChatMessageType[]>(INITIAL_MESSAGES);
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      const catalog = await getCatalogProducts(12);
      setProducts(catalog);
    } catch (error) {
            ...
            <Stack gap="6">
              ...
              {!context.error && (
                <div className="ml-2">
                  ...
                  >
                    {showProducts && productsLoading && (
                      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        {Array.from({ length: 6 }).map((_, index) => (
                          <div
                            key={`product-skeleton-${index}`}
                            className="h-[320px] rounded-xl bg-base animate-pulse"
                          />
                        ))}
                      </div>
                    )}
                    ...
                    {showProducts && productsError && (
                      <ErrorDisplay
                        ...
                        onDismiss={handleRetryProducts}
                      />
                    )}

                    {showProducts && !productsLoading && !productsError && (
                      <ProductGrid
                        products={products}
